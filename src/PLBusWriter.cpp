...
PLBusWriter::PLBusWriter(gpio_num_t pin) : _pin(pin) {}

void PLBusWriter::begin() {
  pinMode(_pin, OUTPUT);
  digitalWrite(_pin, LOW); // idle: transistor off, bus released
}

void PLBusWriter::pulse(uint8_t tcode) {
...
uint8_t PLBusWriter::encodeBit(int lastBit, int bit) {
  if (lastBit == 0) return bit ? 3 : 2;
  return bit ? 2 : 1;
}

void PLBusWriter::sendFrame(const String &bits) {
