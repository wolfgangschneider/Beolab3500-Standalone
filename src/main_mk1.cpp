/*
  Beolab3500-Standalone - MK1 MCL/PL bus Master emulator for Beolab
  3500 startup

  IMPORTANT: only verified against the Beolab 3500 Mk1, which uses the
  B&O MCL/PL "Datalink" bus protocol below. The Mk2 uses a different,
  pure PowerLink protocol entirely - see main_mk2.cpp /
  powerlink_mk2/, do not assume this works unmodified on it.
...
#include <Arduino.h>
#include "mcl_mk1/MclBusReader.hpp"
#include "mcl_mk1/MclBusWriter.hpp"
#include "mcl_mk1/MclData.hpp"
...
static void sendMasterReply(const MclData &frame) {
  uint8_t device = (uint8_t) frame.device;
  //Serial.printf("-> replying for %s(%d)\n", MclData::deviceName(device), device);
  ...
  String test = MclData::buildSelectSourceBits(device);

  writer.sendFrame(MclData::buildSoundBits(78, 6, 68)); // is the range 6 => 0-72  that means 72 = 100% (3 >= 32=100%) // last parameter unknown
  writer.sendFrame(MclData::buildSoundBits(78, 6, 68));
  writer.sendFrame(test);
  writer.sendFrame(test);
 // writer.sendFrame(MclData::buildSoundBits(78, 3, 68));
  GpioOutputs::setActiveSourcePin(device);
}
...
static void handleDebugSerial() {
  ...
  while (Serial.available()) {
    ...
    if (sscanf(line.c_str(), "%d %d %d", &type, &subType, &value) == 3) {
      ...
      writer.sendFrame(MclData::buildSoundBits((uint8_t) type, (uint8_t) subType, (uint8_t) value));
      continue;
    }

    int device = MclData::deviceFromName(line);
    ...
    if (device >= 0) {
      ...
      // data=(device-192)&0x1F), then parse it through MclData's real
      ...
      String bits = "000"; // Format (unused by MclData)
      for (int b = 4; b >= 0; b--) bits += ((0 >> b) & 1) ? '1' : '0';        // AddrTo = 0
      for (int b = 3; b >= 0; b--) bits += ((MclData::BL3500_ADDR >> b) & 1) ? '1' : '0'; // AddrFrom
